...
- (void)setUp
{
  if (!_url) {
    ...
    NSString *port = [standardDefaults valueForKey:@"websocket-executor-port"] ?: @"8081";
    NSString *address = [standardDefaults valueForKey:@"websocket-executor-address"] ?: @"localhost";
    NSString *URLString = [NSString stringWithFormat:@"http://%@:%@/debugger-proxy", address, port];
